...
class TestRealDataWorkflows(unittest.TestCase):
    ...
    def test_load_legend_files(self):
        ...
        stratified_legend = os.path.join(
            self.data_dir, "SmallExample.stratified.legend"
        )
        ...
        fun_count = sum(
            1 for i in range(legend.row_count()) if legend[i]["fun"] == "fun"
        )
        syn_count = sum(
            1 for i in range(legend.row_count()) if legend[i]["fun"] == "syn"
        )
        ...
        self.assertIn(
            "protected", legend.get_header(), "Should have 'protected' column"
        )

        protected_count = sum(
            1 for i in range(legend.row_count()) if legend[i]["protected"] == "1"
        )
    ...
    def test_load_bin_files(self):
        ...
        for bin_tuple in bins:
            self.assertEqual(
                len(bin_tuple), 3, "Each bin should have (lower, upper, expected)"
            )
            self.assertGreaterEqual(
                bin_tuple[1], bin_tuple[0], "Upper bound >= lower bound"
            )

        # Functional bins
        fun_bins = loadBins(
            os.path.join(self.data_dir, "Expected_variants_functional.txt")
        )
        self.assertGreater(len(fun_bins), 0, "Should have functional bins")

        # Synonymous bins
        syn_bins = loadBins(
            os.path.join(self.data_dir, "Expected_variants_synonymous.txt")
        )
    ...
    def test_probabilistic_pruning_complete_workflow(self):
        ...
        output_alleles = sum(
            output_matrix.row_num(i) for i in range(output_matrix.num_rows())
        )
        original_alleles = sum(
            original_matrix.row_num(i) for i in range(original_matrix.num_rows())
        )
        ...
        self.assertLess(
            output_alleles,
            original_alleles,
            "Probabilistic pruning should remove alleles",
        )
    ...
    def test_probabilistic_pruning_with_z_flag(self):
        ...
        self.assertTrue(
            os.path.exists(output_legend), "Output legend file must be created"
        )

        pruned_variants_file = f"{output_legend}-pruned-variants"
        self.assertTrue(
            os.path.exists(pruned_variants_file), "Pruned variants file must be created"
        )
    ...
    def test_standard_pruning_complete_workflow(self):
        ...
        self.assertGreater(
            output_matrix.num_rows(), 0, "Should have variants after pruning"
        )
        ...
        self.assertEqual(
            output_matrix.num_rows(),
            7,
            "With seed=123, should have exactly 7 rows after pruning",
        )
        self.assertEqual(
            output_legend_obj.row_count(), 7, "Legend should match matrix row count"
        )
        self.assertEqual(
            output_matrix.num_cols(),
            original_matrix.num_cols(),
        ...
        self.assertGreater(
            output_matrix.num_rows(), 0, "Must have variants after pruning"
        )
    ...
    def test_functional_split_complete_workflow(self):
        ...
        original_fun = sum(
            1
            for i in range(original_legend.row_count())
        ...
        self.assertEqual(
            output_matrix.num_rows(), 11, "With seed=456, should have exactly 11 rows"
        )
        self.assertEqual(
            output_legend_obj.row_count(), 11, "Legend should match matrix"
        )
...
class TestDataFileIntegrity(unittest.TestCase):
    ...
    def test_haplotype_files_load_correctly(self):
        ...
        haps_files = {"ProbExample.haps": (31, 20), "bigger_test.haps": (80, 55)}
    ...
    def test_compressed_haps_load_correctly(self):
        ...
        compressed_files = ["bigger_test.haps.gz", "Simulated_80k_9.controls.haps.gz"]
        ...
        for filename in compressed_files:
            ...
            if os.path.exists(file_path):
                matrix = reader.loadSparseMatrix(file_path)
                self.assertGreater(matrix.num_rows(), 0, f"{filename} should have rows")
                self.assertGreater(
                    matrix.num_cols(), 0, f"{filename} should have columns"
                )
