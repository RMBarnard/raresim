...
class TestStandardPruner(unittest.TestCase):
    def setUp(self):
        ...
        self.protected_indices = [
            ...
            20,
            25,
            30,  # 5 from bin 0 (AC=1)
            42,
            45,
            48,
            52,
            55,  # 5 from bin 1 (AC=2-5)
            62,
            65,
            68,
            72,
            75,  # 5 from bin 2 (AC=6-10)
            82,
            85,
            88,
            92,
            95,  # 5 from bin 3 (AC>10)
        ]
        ...
        self.legend = legend
        self.matrix = matrix
        self.bins = [[1, 1, 25], [2, 5, 10], [6, 10, 3]]
    ...
    def test_full_transform_no_z(self):
        ...
        assert (
            num_rows_with_0_columns
            + sum(len(after_assigned_bins[entry]) for entry in after_assigned_bins)
            == 100
        )
    ...
    def test_full_transform_with_z(self):
        ...
        total_kept_variants = sum(
            len(after_assigned_bins[entry]) for entry in after_assigned_bins
        )
    ...
    def test_full_transform_with_keep_protected(self):
        ...
        for protected_idx in self.protected_indices:
            if (
                self.matrix.row_num(protected_idx) > 0
            ):  # Only check if variant has AC > 0
                assert (
                    protected_idx in all_kept_rows
                ), f"Protected variant {protected_idx} was pruned!"
        ...
        with open(pruned_file, "r") as f:
            ...
            for line in lines[1:]:  # Skip header
                ...
                assert (
                    idx not in self.protected_indices
                ), f"Protected variant {idx} was written to pruned file!"
    ...
    def test_borrowing_from_extra_rows(self):
        ...
        bins_with_borrowing = [
            ...
            [
                ...
                5,
                30,
            ],  # Bin 1: AC=2-5, need 30 (have 20, need to borrow ~10 from extra_rows)
            [
                6,
                10,
                3,
            ],  # Bin 2: AC=6-10, need 3 (have 20, will prune ~17 into extra_rows, processed first)
        ]
        ...
        pruner = StandardPruner(
            runConfig, bins_with_borrowing, self.legend, self.matrix
        )
        pruner.transform()
        ...
        assert (
            abs(len(after_assigned_bins[1]) - 30) <= 5
        ), f"Bin 1 should have ~30 variants but has {len(after_assigned_bins[1])}"
        ...
        for row_id in after_assigned_bins[1]:
            ...
            assert (
                2 <= row_ac <= 5
            ), f"Variant {row_id} in bin 1 has AC={row_ac}, expected AC in [2, 5]"
    ...
    def test_borrowing_from_reserve_pool(self):
        ...
        bins_with_reserve_borrowing = [
            ...
            [
                ...
                10,
                18,
            ],  # Bin 2: AC=6-10, need 18 (have 20, will prune only ~2 into extra_rows, processed first)
        ]
        ...
        pruner = StandardPruner(
            runConfig, bins_with_reserve_borrowing, self.legend, self.matrix
        )
        pruner.transform()
        ...
        assert (
            len(after_assigned_bins[3]) < 20
        ), f"Reserve pool should have been reduced from borrowing, but has {len(after_assigned_bins[3])}"
    ...
    def test_fun_only_mode(self):
        ...
        assert (
            syn_count == 50
        ), f"Expected exactly 50 synonymous variants to be kept, but got {syn_count}"
    ...
    def test_syn_only_mode(self):
        ...
        assert (
            fun_count == 50
        ), f"Expected exactly 50 functional variants to be kept, but got {fun_count}"
...
class TestFunctionalSplitPruner(unittest.TestCase):
    def setUp(self):
        ...
        random.seed(
            789
        )  # Different seed from StandardPruner tests and integration tests
        header = ["id", "position", "a0", "a1", "protected", "fun"]
        legend = Legend(header)
        matrix = SparseMatrix()
    ...
    def test_assign_bins(self):
        ...
        args = Namespace(
            z=False,
            exp_bins=None,
            exp_fun_bins="some_bins",
            exp_syn_bins="some_bins",
            small_sample=True,
            activation_threshold=5,
            stop_threshold=2,
            keep_protected=False,
        ...
        assert (
            fun_total >= 45 and fun_total <= 50
        ), f"Expected ~50 functional variants, got {fun_total}"
        assert (
            syn_total >= 45 and syn_total <= 50
        ), f"Expected ~50 synonymous variants, got {syn_total}"
    ...
    def test_full_transform_with_z(self):
        ...
        total_kept_variants = sum(
            [len(after_assigned_bins["fun"][i]) for i in after_assigned_bins["fun"]]
        )
        total_kept_variants += sum(
            [len(after_assigned_bins["syn"][i]) for i in after_assigned_bins["syn"]]
        )
    ...
    def test_borrowing_no_crossover(self):
        ...
        bins_with_borrowing = {
            "fun": [
                ...
                [
                    ...
                    10,
                    2,
                ],  # Bin 2: AC=6-10, need 2 (have ~10, will prune ~8 into extra_rows)
            ],
            "syn": [
                [1, 1, 12],  # Bin 0: AC=1, need 12 (have ~17)
                [2, 5, 15],  # Bin 1: AC=2-5, need 15 (have ~10, need to borrow ~5)
        ...
        assert (
            abs(len(after_assigned_bins["fun"][1]) - 15) <= 5
        ), f"Fun bin 1 should have ~15 variants but has {len(after_assigned_bins['fun'][1])}"
        assert (
            abs(len(after_assigned_bins["syn"][1]) - 15) <= 5
        ), f"Syn bin 1 should have ~15 variants but has {len(after_assigned_bins['syn'][1])}"

        # Verify all variants in fun bins are functional
        for bin_id in after_assigned_bins["fun"]:
            for row_id in after_assigned_bins["fun"][bin_id]:
                assert (
                    self.legend[row_id]["fun"] == "fun"
                ), f"Found non-functional variant {row_id} in functional bin {bin_id}"

        # Verify all variants in syn bins are synonymous
        for bin_id in after_assigned_bins["syn"]:
            for row_id in after_assigned_bins["syn"][bin_id]:
                assert (
                    self.legend[row_id]["fun"] == "syn"
                ), f"Found non-synonymous variant {row_id} in synonymous bin {bin_id}"
...
class TestProbabilisticPruner(unittest.TestCase):
    ...
    def test_probabilistic_pruning_removes_whole_rows(self):
        ...
        self.assertEqual(
            self.matrix.row_num(0), 0, "prob=0.0 should always prune the whole variant"
        )
        self.assertEqual(self.matrix.get_row(0), [0, 0, 0, 0])
        self.assertEqual(
            self.matrix.get_row(1),
            [0, 1, 0, 1],
            "prob=1.0 should always keep the variant",
        )
        self.assertEqual(
            self.matrix.get_row(2),
            [1, 0, 1, 0],
            "'.' should leave the variant unchanged",
        )
    ...
    def test_probabilistic_pruning_prints_allele_frequency_distributions(self):
        ...
        output = stdout.getvalue()
        self.assertIn("Allele frequency distribution:", output)
