    ...
    if extension == ".csv":
        with open(macs_file) as macs_file:
            ...
            if header[0].strip() != "Lower" or header[1].strip() != "Upper":
                raise Exception(
                    "Mac bins file needs to have column names Lower and Upper"
                )

            for line in lines[1:]:
                parts = line.strip().split(",")
                macs.append((int(parts[0]), int(parts[1])))
    elif extension == ".txt":
        with open(macs_file) as macs_file:
            lines = macs_file.readlines()
            header = lines[0].split("\t")
...
def fit_nvars(observed_variants_per_kb):
    ...
    if not np.issubdtype(
        observed_variants_per_kb.iloc[:, 0].dtype, np.number
    ) or not np.issubdtype(observed_variants_per_kb.iloc[:, 1].dtype, np.number):
    ...
    if observed_variants_per_kb.iloc[:, 1].isna().any():
        raise ValueError(
            "Number of variants per Kb need to be numeric with no NA values"
        )
    ...
    def leastsquares(inner_tune):
        ...
        sq_err = (
            E - observed_variants_per_kb.iloc[:, 1]
        ) ** 2  # calculate the squared error of expected - observed
    ...
    phi = observed_variants_per_kb.iloc[
        observed_variants_per_kb.iloc[:, 0].idxmax(), 1
    ] / (
        observed_variants_per_kb.iloc[observed_variants_per_kb.iloc[:, 0].idxmax(), 0]
        ** 0.45
    )
    ...
    re_LS = minimize(
        ...
        options={"disp": False, "ftol": 0.0, "maxiter": 100},
    )
    ...
    if re_LS.fun > 1000:
        ...
        for omega in np.arange(
            ...
            phi = observed_variants_per_kb.iloc[
                ...
                observed_variants_per_kb.iloc[
                    observed_variants_per_kb.iloc[:, 0].idxmax(), 0
                ]
                ** omega
            )
            ...
            re_LS1 = minimize(
                ...
                constraints=constraints,
                options={"disp": False, "ftol": 0.0, "maxiter": 100},
            )  # estimate parameters with SLSQP
            to_bind1 = np.concatenate(
                (re_LS1.x, [re_LS1.fun])
            )  # record parameters and loss value
            re_tab1.append(to_bind1)  # bind information from each iteration together

        re_tab1 = np.array(re_tab1)
        re_fin = re_tab1[
            np.argmin(re_tab1[:, 2])
        ]  # select the minimum least squared error
        phi = re_fin[0]
        omega = re_fin[1]
    ...
    print(
        f"Calculated the following params from nvar target data. omega: {omega:.4f}, phi: {phi:.4f}"
    )
    return omega, phi
...
def fit_afs(observed_bin_props_df):
    ...
    if list(observed_bin_props_df.columns[:3]) != ["Lower", "Upper", "Prop"]:
        raise Exception(
            "Observed_bin_props needs to have column names Lower, Upper, and Prop"
        )

    # Make sure the Observed_bin_props are numeric
    observed_bin_props_df["Prop"] = pd.to_numeric(
        observed_bin_props_df["Prop"], errors="raise"
    )

    # Make sure there are not any NAs in the proportions
    if observed_bin_props_df["Prop"].isnull().any():
        raise Exception(
            "Proportions in Observed_bin_props need to be numeric with no NA values"
        )

    if not pd.api.types.is_numeric_dtype(
        observed_bin_props_df["Lower"]
    ) or not pd.api.types.is_numeric_dtype(observed_bin_props_df["Upper"]):
        raise Exception("Observed_bin_props MAC bins need to be numeric")

    # Check the order of the MAC bins
    if (
        not observed_bin_props_df["Upper"].is_monotonic_increasing
        or not observed_bin_props_df["Lower"].is_monotonic_increasing
    ):
    ...
    c1 = np.arange(
        1, int(upper_last) + 1
    )  # creates a sequence from 1 to the last Upper value
    ...
    S = minimize(
        ...
        tune,
        constraints=cons,
        options={"disp": False, "ftol": 0.0, "maxiter": 25},
    )
    ...
    b = b
    print(
        f"Calculated the following params from AFS target data. alpha: {alpha:.4f}, beta: {beta:.4f}, b: {b:.4f}"
    )
    return alpha, beta, b
...
def calc(args):
    ...
    if args.pop:
        if args.pop.strip() not in DEFAULT_PARAMS.keys():
            raise Exception(
                f"{args.pop} is not a valid population. Valid populations are: {','.join(DEFAULT_PARAMS.keys())}"
            )
    elif (args.nvar_target_data is None or args.afs_target_data is None) and (
        args.alpha is None
        or args.beta is None
        or args.omega is None
        or args.phi is None
        or args.b is None
    ):
        raise Exception(
            "Error: either a default population should be specified or alpha, beta, omega, phi, and b parameters provided or target values for nvars and afs should be provided"
        )
    is_stratified = False
    ...
    if not is_stratified:
        ...
        if args.nvar_target_data is None or args.afs_target_data is None:
            raise Exception(
                "Error: stratification is currently only supported when target data is provided for nvars and afs"
            )
        ...
        syn_weight = max(
            0.0, min(float(args.w_syn), 2.0)
        )  # clamp the weight to be in the range [0.2]
